...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="get-GUF10-is-service">
    <xsl:value-of
            select="'false'"/>
  </xsl:template>

  <xsl:template name="get-GUF10-title">
    <xsl:value-of select="$metadata/mdb:identificationInfo/*/mri:citation/*/cit:title/gco:CharacterString"/>
  </xsl:template>

  <xsl:template name="get-GUF10-extents-as-json">[
  ...
  <xsl:template name="get-GUF10-online-source-config">
