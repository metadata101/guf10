...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template mode="mode-GUF10"
                match="guf:*|mds:*|mcc:*|mri:*|mrs:*|mrc:*|mrd:*|mco:*|msr:*|lan:*|
  ...
  <xsl:template mode="mode-GUF10" match="gn:*|@gn:*|@*" priority="1000"/>
  ...
  <xsl:template mode="mode-GUF10" match="gn:child" priority="2000">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
  ...
  -->
  <xsl:template mode="mode-GUF10" priority="200"
  ...
  <!--
  <xsl:template mode="mode-GUF10" match="*|@*" priority="0"/>
-->
  <!-- Codelists -->
  <xsl:template mode="mode-GUF10" priority="400"
                match="*[*/@codeList]">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    ...
    <xsl:call-template name="render-element">
      ...
      <xsl:with-param name="cls" select="local-name()"/>
      <xsl:with-param name="xpath" select="$xpath"/>
      <xsl:with-param name="type" select="gn-fn-guf10:getCodeListType(name(), $editorConfig)"/>
  ...
  next template -->
  <xsl:template mode="mode-GUF10" priority="400" match="gml:TimeInstantType"/>

  <!-- the gml element having no child eg. gml:name. -->
  <xsl:template mode="mode-GUF10" priority="300" match="gml:*[count(.//gn:element) = 1]">
