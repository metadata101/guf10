...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <cit:CI_Citation>
  <cit:title>
    <gco:CharacterString>Towards the global monitoring of biodiversity
      change</gco:CharacterString>
  </cit:title>
  <cit:date>

  <xsl:template mode="csv" match="guf:GUF_FeedbackItem|*[@gco:isoType='GUF_FeedbackItem']"
                priority="2">
    <metadata>
      <xsl:variable name="langId" select="gn-fn-guf10:getLangId(., $lang)"/>
      <id>
      ...
      <title>
        <xsl:apply-templates mode="localised"
                             select="guf:citation/*/cit:title">
          <xsl:with-param name="langId" select="$langId"/>
        </xsl:apply-templates>
      </title>
      <abstract>
        <xsl:apply-templates mode="localised" select="guf:abstract">
          <xsl:with-param name="langId" select="$langId"/>
        </xsl:apply-templates>
      </abstract>
      ...
      <metadatacreationdate>
        <xsl:value-of select="guf:dateInfo/*/cit:date/*"/>
      </metadatacreationdate>

      <xsl:for-each select="guf:citation/*/cit:date">
      ...
      <xsl:for-each select="guf:descriptiveKeywords/*[not(mri:thesaurusName)]/mri:keyword[not(@gco:nilReason)]">
        <keyword>
      ...
      <xsl:for-each select="guf:descriptiveKeywords/*[mri:thesaurusName]">
      ...
      <xsl:for-each select="guf:contact">
