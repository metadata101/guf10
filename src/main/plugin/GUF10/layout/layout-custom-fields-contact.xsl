...
<xsl:stylesheet version="2.0"
  ...
  -->
  <xsl:template mode="mode-GUF10" match="*[cit:CI_Telephone]" priority="2000">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>

    <xsl:apply-templates mode="mode-GUF10" select="*/cit:*">
  ...
  <xsl:template mode="mode-GUF10" match="cit:numberType" priority="2000"/>
