...
<xsl:stylesheet version="2.0"
  ...
  <!-- Custom rendering of keyword section
    ...
    * if the thesaurus is available in the catalog, display
    ...
    keywords.
  -->
  <xsl:template mode="mode-GUF10" priority="2000" match="
    guf:descriptiveKeywords">
  ...
  <xsl:template mode="mode-GUF10" match="mri:MD_Keywords" priority="2000">
    <xsl:variable name="thesaurusTitle"
      ...
      <xsl:when test="$isTheaurusAvailable">
        ...
        <xsl:variable name="transformations"
                      ...
                      select="if ($thesaurusConfig/@transformations != '')
                              ...
                              else 'to-guf10-keyword,to-guf10-keyword-with-anchor,to-guf10-keyword-as-xlink'"/>

        <!-- Get current transformation mode based on XML fragement analysis -->
        <xsl:variable name="transformation"
          select="if (parent::node()/@xlink:href)
                  then 'to-guf10-keyword-as-xlink'
                  else if (count(mri:keyword/gmx:Anchor) > 0)
                  then 'to-guf10-keyword-with-anchor'
                  else 'to-guf10-keyword'"/>
