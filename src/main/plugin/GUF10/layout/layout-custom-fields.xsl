...
-->
  <xsl:template mode="mode-GUF10"
                match="guf:itemIdentifier/mcc:MD_Identifier/mcc:code|
                       guf:itemIdentifier/mcc:MD_Identifier/mcc:codeSpace|
                       guf:itemIdentifier/mcc:MD_Identifier/mcc:description|
                       guf:dateInfo/cit:CI_Date[cit:dateType/cit:CI_DateTypeCode = 'revision']/cit:date|
                       guf:dateInfo/cit:CI_Date[cit:dateType/cit:CI_DateTypeCode = 'revision']/cit:dateType"
                priority="2000">
 ...
 <!-- <xsl:template mode="mode-GUF10"
    ...
    <div class="row">
      <div class="col-md-8">
        <xsl:apply-templates mode="mode-GUF10" select="cit:date">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="labels" select="$labels"/>
        </xsl:apply-templates>
      </div>
      <div class="col-md-4">
        <xsl:apply-templates mode="mode-GUF10" select="cit:dateType">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="labels" select="$labels"/>
        </xsl:apply-templates>
      </div>
  ...
  <xsl:template mode="mode-GUF10" match="gml:beginPosition|gml:endPosition|gml:timePosition"
                priority="200">
  ...
  <xsl:template mode="mode-GUF10"
                ...
                priority="2000">
    <xsl:apply-templates mode="mode-GUF10" select="*"/>
  </xsl:template>
