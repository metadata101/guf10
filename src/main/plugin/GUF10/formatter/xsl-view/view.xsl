...
<xsl:stylesheet version="2.0"
  ...
  * render-value taking care of element values (eg. characterString, URL)
  ...
  <xsl:variable name="metadata"
                select="/root/guf:GUF_FeedbackItem"/>




  <!-- Specific schema rendering -->
  <xsl:template mode="getMetadataTitle" match="guf:GUF_FeedbackItem">
    <xsl:variable name="value"
                  select="guf:citation/*/cit:title"/>
    <xsl:value-of select="$value/gco:CharacterString"/>
  </xsl:template>

  <xsl:template mode="getMetadataAbstract" match="guf:GUF_FeedbackItem">
    <xsl:variable name="value"
                  select="guf:abstract"/>
    <xsl:value-of select="$value/gco:CharacterString"/>
  </xsl:template>
  ...
  <xsl:template mode="getMetadataHeader" match="guf:GUF_FeedbackItem">
  </xsl:template>
  ...
  <xsl:template mode="render-field"
                match="*[count(guf:*|mdb:*|mri:*|mcc:*|dqm:*|mco:*|mrc:*|
  ...
  <xsl:template mode="render-field"
                match="guf:descriptiveKeywords[count(*/mri:keyword) = 0]" priority="200"/>
  <xsl:template mode="render-field"
                match="guf:descriptiveKeywords[
  ...
  <xsl:template mode="render-field"
                match="guf:descriptiveKeywords[not(*/mri:thesaurusName/cit:CI_Citation/cit:title)]"
                priority="100">
    <dl class="gn-keyword">
      <dt>
